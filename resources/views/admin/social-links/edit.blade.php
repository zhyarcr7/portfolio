                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('admin.social-links.update', $socialLink->id) }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="platform" name="platform" type="text" class="mt-1 block w-full" :value="old('platform', $socialLink->platform)" required autofocus />
                        ...
                        <div>
                            ...
                            <x-text-input id="url" name="url" type="text" class="mt-1 block w-full" :value="old('url', $socialLink->url)" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="icon_class" name="icon_class" type="text" class="mt-1 block w-full" :value="old('icon_class', $socialLink->icon_class)" required />
                            <x-input-error :messages="$errors->get('icon_class')" class="mt-2" />
                            <div class="flex items-center space-x-2 mt-2">
                                <span>Current icon:</span>
                                <i class="{{ $socialLink->icon_class }} text-[#FF750F] text-xl"></i>
                            </div>
                        ...
                        <div>
                            ...
                            <x-text-input id="order" name="order" type="number" class="mt-1 block w-full" :value="old('order', $socialLink->order)" />
                            <x-input-error :messages="$errors->get('order')" class="mt-2" />
                            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Lower numbers will appear first</p>
                        </div>

                        <div class="flex items-center">
                            <input id="is_active" name="is_active" type="checkbox" class="rounded dark:bg-blue-900/90 border-gray-300 dark:border-gray-700 text-[#FF750F] shadow-sm focus:ring-[#FF750F] dark:focus:ring-[#FF750F] dark:focus:ring-offset-gray-800" {{ $socialLink->is_active ? 'checked' : '' }}>
